...
protected:
    ...
    virtual bool insert(BTreeNode<T>* node, BTreeNode<T>* fathernode, BTNodePos pos)
    {
        bool ret = true;
        if( pos == ANY )
        {
    ...
    {
        return node ? ((height(node->left) < height(node->right))? (height(node->right) + 1) : (height(node->left) + 1)) : 0;
    }
    int degree(BTreeNode<T>* node)const
    {
        ...
        if(node != NULL)
        {
            ...
            */
            ...
            if( ret < 2 )
            {
                int d = (degree(node->left) < degree(node->right)) ? degree(node->right) : degree(node->left);
...
public:
    ...
    virtual bool insert(TreeNode<T>* node, BTNodePos pos)
    {
        ...
        if( node != NULL )
        {
            ...
            else
            {
                ...
                {
                    THROW_EXCEPTION(InvalidParameterException, "No node to insert at current tree ...");
                }
            }
        }
        else
    ...
    SharedPointer<Array<T>> traversal(BTTraversal order)
    {
        ...
        {
            THROW_EXCEPTION(NoEnoughMemoryException, "No enough memory to creare dynamic array ...");
        }
        return ret;
    }
    ...
    SharedPointer<BTree<T>> add(const BTree<T>& btree) const
    {
        ...
        {
            THROW_EXCEPTION(NoEnoughMemoryException, "No enough to create tree ...");
        }
        return ret;
    }
